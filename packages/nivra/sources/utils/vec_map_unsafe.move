...
public struct VecMapUnsafe<K: copy, V> has copy, drop, store {
...
/// Create an empty `VecMapUnsafe`
public fun empty<K: copy, V>(): VecMapUnsafe<K, V> {
    VecMapUnsafe { contents: vector[] }
}

/// Insert the entry `key` |-> `value` into `self`.
/// Aborts if `key` is already bound in `self`.
public fun insert<K: copy, V>(self: &mut VecMapUnsafe<K, V>, key: K, value: V) {
    assert!(!self.contains(&key), EKeyAlreadyExists);
    self.contents.push_back(Entry { key, value })
}

public fun insert_unsafe<K: copy, V>(self: &mut VecMapUnsafe<K, V>, key: K, value: V) {
    self.contents.push_back(Entry { key, value })
}

/// Remove the entry `key` |-> `value` from self. Aborts if `key` is not bound in `self`.
public fun remove<K: copy, V>(self: &mut VecMapUnsafe<K, V>, key: &K): (K, V) {
...
public fun pop<K: copy, V>(self: &mut VecMapUnsafe<K, V>): (K, V) {
...
public fun get_mut<K: copy, V>(self: &mut VecMapUnsafe<K, V>, key: &K): &mut V {
    let idx = self.get_idx(key);
...
/// Aborts if `key` is not bound in `self`.
public fun get<K: copy, V>(self: &VecMapUnsafe<K, V>, key: &K): &V {
    let idx = self.get_idx(key);
...
public fun try_get<K: copy, V: copy>(self: &VecMapUnsafe<K, V>, key: &K): Option<V> {
...
public fun contains<K: copy, V>(self: &VecMapUnsafe<K, V>, key: &K): bool {
    get_idx_opt(self, key).is_some()
}

/// Return the number of entries in `self`
public fun length<K: copy, V>(self: &VecMapUnsafe<K, V>): u64 {
    self.contents.length()
}

/// Return true if `self` has 0 elements, false otherwise
public fun is_empty<K: copy, V>(self: &VecMapUnsafe<K, V>): bool {
    self.length() == 0
}

/// Destroy an empty map. Aborts if `self` is not empty
public fun destroy_empty<K: copy, V>(self: VecMapUnsafe<K, V>) {
    let VecMapUnsafe { contents } = self;
...
public fun into_keys_values<K: copy, V>(self: VecMapUnsafe<K, V>): (vector<K>, vector<V>) {
    let VecMapUnsafe { contents } = self;
    let mut keys = vector[];
...
/// Construct a new `VecMapUnsafe` from two vectors, one for keys and one for values.
...
/// and are *not* sorted.
public fun from_keys_values<K: copy, V>(keys: vector<K>, values: vector<V>): VecMapUnsafe<K, V> {
    assert!(keys.length() == values.length(), EUnequalLengths);
    let mut contents = VecMapUnsafe { contents: vector[] };
    keys.zip_do!(values, |key, value| contents.insert(key, value));
    contents
}

/// Returns a list of keys in the map.
/// Do not assume any particular ordering.
public fun keys<K: copy, V>(self: &VecMapUnsafe<K, V>): vector<K> {
...
public fun get_idx_opt<K: copy, V>(self: &VecMapUnsafe<K, V>, key: &K): Option<u64> {
    self.contents.find_index!(|Entry { key: k, .. }| k == key)
}

/// Find the index of `key` in `self`. Aborts if `key` is not in `self`.
/// Note that map entries are stored in insertion order, *not* sorted by key.
public fun get_idx<K: copy, V>(self: &VecMapUnsafe<K, V>, key: &K): u64 {
    self.contents.find_index!(|Entry { key: k, .. }| k == key).destroy_or!(abort EKeyDoesNotExist)
}
...
public fun get_entry_by_idx<K: copy, V>(self: &VecMapUnsafe<K, V>, idx: u64): (&K, &V) {
...
/// Note that map entries are stored in insertion order, *not* sorted by key.
/// Aborts if `idx` is greater than or equal to `self.length()`
public fun get_entry_by_idx_mut<K: copy, V>(self: &mut VecMapUnsafe<K, V>, idx: u64): (&K, &mut V) {
    assert!(idx < self.length(), EIndexOutOfBounds);
    let entry = &mut self.contents[idx];
    (&entry.key, &mut entry.value)
}

public fun get_value_by_idx_mut<K: copy, V>(self: &mut VecMapUnsafe<K, V>, idx: u64): &mut V {
    assert!(idx < self.length(), EIndexOutOfBounds);
    let entry = &mut self.contents[idx];
    &mut entry.value
}

public fun get_value_by_idx<K: copy, V>(self: &VecMapUnsafe<K, V>, idx: u64): &V {
    assert!(idx < self.length(), EIndexOutOfBounds);
    let entry = &self.contents[idx];
    &entry.value
}
...
/// Aborts if `idx` is greater than or equal to `self.length()`
public fun remove_entry_by_idx<K: copy, V>(self: &mut VecMapUnsafe<K, V>, idx: u64): (K, V) {
    assert!(idx < self.length(), EIndexOutOfBounds);
    let Entry { key, value } = self.contents.remove(idx);
    (key, value)
}

public macro fun for_each<$K, $V: copy>(
    $v: &mut VecMapUnsafe<$K, $V>,
    $f: |&$K, &mut $V| -> (),
) {
    let mut i = 0;

    while(i < length($v)) {
        let (k, v) = get_entry_by_idx_mut($v, i);
...
public macro fun for_each_ref<$K, $V: copy>(
    $v: &VecMapUnsafe<$K, $V>,
    $f: |&$K, &$V| -> (),
) {
    let mut i = 0;
